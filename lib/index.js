...
class FlowNode {
  constructor(astNode, parent, type) {
    ...
    if (astNode !== undefined) {
      id += hash(astNode.type) + hash(astNode.range);
    }
    if (type !== undefined) {
      id += hash(type);
    }
...
/**
 * Returns [[entryNode, exitNode, allNodes], ...] `FlowNode`s for the passed in AST and child functions
 */
function ControlFlowGraph(astNode) {
  const parentStack = [[]];
  const exitNode = new FlowNode(astNode, undefined, 'exit');
  const catchStack = [[exitNode]];
  const entryNode = new FlowNode(astNode, undefined, 'entry');
  const functions = [];

  createNodes(astNode);
  // astNode.cfg = entryNode;
  ...
  walker(astNode, {
    ...
    FunctionDeclaration(node, recurse) {
      if (astNode !== node) {
        connectNode(node.cfg);
        pushLastVisitedNode(node.cfg, 'normal');
        functions.push(...ControlFlowGraph(node));
      } else {
        recurse(node.body);
      }
    },
    FunctionExpression(node, recurse) {
      if (astNode !== node) {
        connectNode(node.cfg);
        pushLastVisitedNode(node.cfg, 'normal');
        functions.push(...ControlFlowGraph(node));
    ...
    ArrowFunctionExpression(node, recurse) {
      if (astNode !== node) {
        connectNode(node.cfg);
        pushLastVisitedNode(node.cfg, 'normal');
        functions.push(...ControlFlowGraph(node));
    ...
    WhileStatement(node, recurse) {
      ...
      lastVisitedNode.push(...testNode);
      lastVisitedNode.push(...popLastBreakNode(findLabel(node)));
      console.log('WhileStatement', lastVisitedNode);
    },
    ...
    BreakStatement(node, recurse) {
      ...
      console.log('test', lastBreakNode);
    },
  ...
  walker(astNode, {
    default(node, recurse) {
      if (node.cfg !== undefined && node.cfg.next === undefined && node.cfg.type !== 'exit') {
        delete node.cfg;
      }
  ...
  function popLastBreakNode(label) {
    ...
    console.log('popLastBreakNode', ret);
    return ret;
  }
  ...
  return [[entryNode, exitNode, allNodes], ...functions];
}
